...
Phase 3: Finish  → hand off verification steps, update spec, commit, and wrap up
```
...
- 2.2 User verification handoff `[required · repeatable]`
...
Tools: `trellis-implement` / `trellis-research` are sub-agent types only (Task/Agent tool, NOT Skill; there is no skill by these names). `trellis-update-spec` is a skill. `trellis-check` exists as both but is opt-in only when the user explicitly asks for AI verification or code review.
Flow: `trellis-implement` -> user verification handoff -> `trellis-update-spec` -> commit (Phase 3.4) -> `/trellis:finish-work`.
Main-session default: dispatch implement sub-agents only when needed. Do not dispatch `trellis-check` unless the user explicitly asks for AI verification/code review. Sub-agent self-exemption: if already running as `trellis-implement`, do NOT spawn another `trellis-implement` or `trellis-check`; if already running as `trellis-check`, do NOT spawn another `trellis-check` or `trellis-implement`. Dispatch is main session only.
...
Flow: `trellis-before-dev` -> edit -> user verification handoff -> `trellis-update-spec` -> commit (Phase 3.4) -> `/trellis:finish-work`.
Do not dispatch implement/check sub-agents in inline mode. After editing, do not run full validation or code review unless the user explicitly asks; instead tell the user exactly how to verify the feature themselves.
Read context: `prd.md` -> `design.md if present` -> `implement.md if present`, plus relevant spec/research loaded by skills.
[/workflow-state:in_progress-inline]

### Phase 3: Finish
- 3.1 User verification handoff `[required · repeatable]`
...
- `in_progress` implementation -> dispatch `trellis-implement` if needed; AI check/review requires explicit user request.
...
- Planning or unclear requirements -> `trellis-brainstorm`.
- Before editing -> `trellis-before-dev`; after editing -> give the user verification steps.
- Repeated debugging -> `trellis-break-loop`; spec updates -> `trellis-update-spec`.
...
- Planning must be persisted to task artifacts; after code changes, report that AI verification was not run and provide a concise manual verification checklist for the user.
...
Goal: turn reviewed planning artifacts into code and hand the user a clear manual verification checklist.
...
- **Task description**: Implement the reviewed task artifacts, consulting materials under `{TASK_DIR}/research/`; finish by reporting changed files and manual verification steps for the user
...
5. Do not run full validation or code review unless the user explicitly asks; report manual verification steps instead

#### 2.2 User verification handoff `[required 路 repeatable]`

[Claude Code, Cursor, OpenCode, codex-sub-agent, Kiro, Gemini, Qoder, CodeBuddy, Copilot, Droid, Pi]

Do not spawn the check sub-agent by default. Only spawn it when the user explicitly asks for AI verification or code review.
...
When explicitly requested, the check agent's job:
- Review code changes against specs
- Review code changes against `prd.md`, `design.md` if present, and `implement.md` if present
- Auto-fix issues it finds
- Run lint and typecheck to verify

Default handoff when the user did not ask for AI verification:
- State that AI verification/code review was not run
- List changed files at a high level
- Provide concise manual steps the user can follow to verify the feature

[/Claude Code, Cursor, OpenCode, codex-sub-agent, Kiro, Gemini, Qoder, CodeBuddy, Copilot, Droid, Pi]

[codex-inline, Kilo, Antigravity, Windsurf]

Do not load `trellis-check` by default. Unless the user explicitly asks for AI verification or code review:
- State that AI verification/code review was not run
- List changed files at a high level
- Provide concise manual steps the user can follow to verify the feature
...
- User verification or an explicit AI check reveals a prd defect -> return to Phase 1, fix `prd.md`, then redo 2.1
...
Goal: hand off verification, capture lessons, record the work.

#### 3.1 User verification handoff `[required 路 repeatable]`

Do not run final AI verification or code review unless the user explicitly asks. Before moving on:
- State that AI verification/code review was not run
- Provide the exact manual checks the user should run for this feature
- Ask the user to report any failure details if the manual verification does not pass
